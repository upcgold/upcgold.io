...
contract UPCGoldBank { 
    ...
    */
    ...
    function getScannableByUpc(string memory upcId) public view returns(UpcMeta memory)  {
        bytes32 upcHash = sha256(abi.encodePacked(upcId));
        return scannables[upcHash];
    }
    ...
    function setNftContract(address nftContract) public  {
        upcNft = UPCNFT(nftContract);
    }
    
    
 
     
    function setHumanReadableName(string memory word, string memory _humanReadableName) public  {
        UpcMeta memory upc = getScannableByUpc(word);
        bytes32 upcHashLocal = upc.upcHash;
        
        
        require(msg.sender == scannables[upcHashLocal].staker, "Only the staker can withdraw funds" );
        scannables[upcHashLocal].humanReadableName = _humanReadableName;
        
        for (uint i = 0; i<addressToUpc[msg.sender].length-1; i++){
            bytes32 hashTemp = addressToUpc[msg.sender][i].upcHash;
            if( hashTemp == upcHashLocal ) {
                addressToUpc[msg.sender][i].humanReadableName = _humanReadableName;
            }
        }
        
        upcNft.setHumanReadableName(msg.sender, upcHashLocal, _humanReadableName);
    ...
    function mintNft(address staker, bytes32 upcHash, string memory upcRaw, string memory ipfs, string memory humanReadableName) external returns (uint256) {
        return upcNft.mintNft(staker,upcHash,upcRaw,ipfs,humanReadableName);
    }
    


    function depositMoney(string memory upcId, string memory gameId,  string memory humanReadableName ) public payable {
        bytes32 upcHash = sha256(abi.encodePacked(upcId));
        ...
        upcMeta.humanReadableName = humanReadableName;
        ...
        upcNft.mintNft(msg.sender, upcHash, upcId, defaultIpfs, humanReadableName);
    ...
    function withdraw(string memory upcId) public {
        ...
        scannables[upcHash].isOwned =  false;
